# -*- coding: utf-8 -*-
"""
Created on Wed Oct 22 11:05:38 2025
...
parameters = {
    ...
    'SW_lon': 0., 'SW_lat': 0., # lon and lat at south-west corner of source matrix
    'dlon': 5, 'dlat': 5,  # lon and lat step of source matrix
    'nnx': 1, 'nny': 5, # source point nums on lon and lat     # For zonal mean basic flow, the number of longitudes must be 
                                                               # greater than or equal to 2 for sake of derivatives
    'zwn': np.array([1., 2., 3., 4., 5.]), # initial zonal wave number
    'nzwn': 5, # zonal wave number, decided by len(zwn)
    'tstep': 1, # time step, unit: hour
    'ttotal': 30., # unit: day
    'mode': 'numpy', 
    'root_method': 'numpy', 
    ...
    'inputuv' : 'D:/data/to/your/ideal basic flow.nc', # the basic flow
...
def real2d_hnf(
    ...
    # if zwn is None:
    #     zwn = np.arange(1, nzwn + 1)  # zwn should be typed by users 
    # freq = - 1.D0 / ( 5.D0 * day)
    nsource = nnx * nny
    wr1 = WR(
        nzwn,
    ...
    # SW_lon, SW_lat, dlon, dlat, nnx, nny
    wr1.ray_info()
